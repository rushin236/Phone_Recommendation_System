...
class DataTransformPipeline:
    ...
    def main(self):
        ...
        df_trans, df_cate = data_transform.transform_dataframe(
            df_cate=df_cate, df_trans=df_trans
        )
        df_cate["price"] = df["price"]
        cv = CountVectorizer()
        df_cv = cv.fit_transform(df_trans["text"])
        df_dtm = pd.DataFrame.sparse.from_spmatrix(
            df_cv, columns=cv.get_feature_names_out()
        )
